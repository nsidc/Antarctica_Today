...
def compute_ice_masks(
    input_coastline_tif=default_input_coastline_tif,
    ...
    if ds_coastline is None:
        raise FileNotFoundError(
            "Gdal could not read input file '{0}'".format(input_coastline_tif)
        )
    ...
    if ds_rock is None:
        raise FileNotFoundError(
            "Gdal could not read input file '{0}'".format(input_bedrock_outcrop_tif)
        )
    ...
    for i in range(array_out_shape[0]):
        for j in range(array_out_shape[1]):
            ...
            subarray_coast = array_coastline[
                i * 100 : ((i + 1) * 100), j * 100 : ((j + 1) * 100)
            ]
            subarray_rock = array_rock[
                i * 100 : ((i + 1) * 100), j * 100 : ((j + 1) * 100)
            ]
            ...
            subarray_coast = numpy.logical_or(
                (subarray_coast != ndv_coastline), (subarray_rock != ndv_rock)
            )
    ...
    for outname, outarray in zip(
        ("ice_fraction", "rock_fraction", "ocean_fraction", "ice_mask"),
        ...
        ds = driver.Create(
            ...
            outarray.shape[0],
            1,
            gdal.GDT_Byte if outname == "ice_mask" else gdal.GDT_Float32,
        )
