...
def resize_tif_to_reference_grid(gtif_in, gtif_reference, gtif_out, verbose=False):
    ...
    if ds_ref is None:
        raise FileNotFoundError(
            "Gdal could not read reference file '{0}'".format(gtif_reference)
        )
    ...
    if not (
        geotransform_in[1:3] == geotransform_ref[1:3]
        and geotransform_in[4:6] == geotransform_ref[4:6]
    ):
    ...
    array_out = (
        numpy.zeros(array_ref.shape, dtype=array_in.dtype) + band_in.GetNoDataValue()
    )
    ...
    # Copy the data over.
    array_out[
        y_offset : y_offset + array_in.shape[0], x_offset : x_offset + array_in.shape[1]
    ] = array_in

    # Create the output dataset
    driver = ds_in.GetDriver()
    ds_out = driver.Create(
        gtif_out, array_out.shape[1], array_out.shape[0], 1, band_in.DataType
    )
    if ds_out is None:
...
def read_and_parse_args():
    parser = argparse.ArgumentParser(
        description="Resizes a geotif to have the same boundaries as a reference geotif, and outputs the newly-sized output geotif. CONDITION: The two geotifs should already be the same grid resolution and offset. This doesn't do any resampling, just resizes the grids."
    )
