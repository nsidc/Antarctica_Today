...
axes[0].plot(min_max_elev,
             coefs['REMA_or_Thermap_Elev']*min_max_elev + coefs['const'],
             color='black')
axes[0].text(0.98, 0.98, "temp_C = {0:0.6f}$\cdot$elev\n{1:s} {2:0.4f}".format(
    coefs['REMA_or_Thermap_Elev'], plus_minus_op(coefs['const']), numpy.abs(coefs['const'])),
    transform=axes[0].transAxes, horizontalalignment='right', verticalalignment='top',
    fontsize='large')
axes[0].text(0.05, 0.06, "R$^2$ = {0:0.2f}".format(model_elev_only.rsquared),
             transform=axes[0].transAxes, horizontalalignment='left',
             fontsize='large')
...
# Linear trend-line
axes[2].plot(min_max_elev,
              (coefs['REMA_or_Thermap_Elev']*numpy.array(min_max_elev)) + (-75*coefs['Lat(S)']) + (coefs['const']),
              color='black')

axes[2].set_title("Temp vs. Elev (corrected to 75 S lat)")
axes[2].set_xlabel("Elevation (m)")

axes[2].text(0.98, 0.98, "temp_C = {0:0.6f}$\cdot$elev\n{3:s} {1:0.4f}$\cdot$latitude\n{4:s} {2:0.4f}".format(
    coefs['REMA_or_Thermap_Elev'], numpy.abs(coefs['Lat(S)']), numpy.abs(coefs['const']),
    plus_minus_op(coefs['Lat(S)']), plus_minus_op(coefs['const'])),
    transform=axes[2].transAxes, horizontalalignment='right', verticalalignment='top',
    fontsize='large')
axes[2].text(0.05, 0.06, "R$^2$ = {0:0.2f}".format(model.rsquared),
             transform=axes[2].transAxes, horizontalalignment='left',
             fontsize='large')
