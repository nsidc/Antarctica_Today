...
for idx, row in thermap_df.iterrows():
    ...
    return_line = os.popen("gdallocationinfo -wgs84 -valonly {0} {1} {2}".format(
        REMA_DEM, row['Lon(W)'], row['Lat(S)']))
    try:
        print("{0:>30s} {1:0.2f} {2:0.2f}, {4:0.2f}*C, {3:0.1f} -> ".format(row['Name'], row['Lat(S)'], row['Lon(W)'], row['Elev'], row['Temp']), end='')
...
if EGM96:
    ...
    fig_outfile = os.path.join(os.path.split(THERMAP_CSV)[0], "Thermap_vs_REMA_elevations_EGM96.png")
else:
    output_csv = base + "_REMA" + ext
    fig_outfile = os.path.join(os.path.split(THERMAP_CSV)[0], "Thermap_vs_REMA_elevations.png")
