...
def write_gtif_to_binary(
    ...
    if rasterband > ds.RasterCount:
        raise ValueError(
            "File {0} contains {1} raster bands. Cannot read raster band {2}.".format(
                os.path.split(gtif_filename)[-1], ds.RasterCount, rasterband
            )
        )
...
def read_and_parse_args():
    parser = argparse.ArgumentParser(
        description="Outputs a flat binary integer files (in the style of NSIDC SMMI Polar Stereo Brightness Temperature data) from a GeoTiff."
    )
    parser.add_argument("gtif", type=str, help="Source file (.tif)")
    parser.add_argument(
        "-output", "-o", type=str, default="", help="Destination file (.bin)"
    )
    parser.add_argument(
        "-band",
        "-b",
        type=int,
        default=1,
        help="Raster band number in TIF file. Any integer from 1 to the number of available rastrer bands. Defaults to 1.",
    )
    parser.add_argument(
        "-numbytes",
        "-nb",
        type=int,
        default=2,
        help="Number of output bytes. Can be 1,2,4,8. (Default: 2)",
    )
    parser.add_argument(
        "-multiplier",
        "-m",
    ...
    parser.add_argument(
        ...
        type=str,
        default="little",
        help="Byte-order of the integer values to write.\n"
