...
def retrieve_ssmi_grid_coords(N_or_S="S", gridsize_km=25):
    ...
    x_vector = numpy.arange(
        UL_corner[0], UL_corner[0] + (gridsize_km * gridsize_yx[1]), step=gridsize_km
    )
    y_vector = numpy.arange(
        UL_corner[1], UL_corner[1] + (-gridsize_km * gridsize_yx[0]), step=-gridsize_km
    )
...
def get_hemisphere_and_resolution_from_ssmi_filename(fname):
    ...
    resolution = {19: 25.0, 22: 25.0, 37: 25.0, 85: 12.5, 91: 12.5}[frequency]

    return hemisphere, resolution


def output_bin_to_gtif(
    bin_file,
    gtif_file=None,
    element_size=2,
    header_size=0,
    ...
    if resolution is None or hemisphere is None:
        ...
        (
            hemisphere_from_fname,
            resolution_from_fname,
...
def output_gtif(
    ...
    """
    geotransform = get_nsidc_geotransform(hemisphere=hemisphere, resolution=resolution)
    ...
    if array.dtype in (numpy.int8, numpy.int16, numpy.int32, numpy.int64):
        ...
        raise TypeError(
            "Unhandled data type {0}. Please use int or float.".format(str(array.dtype))
        )
...
def read_and_parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Destination file (.tif). Default: Write the same filename in the same location with a .tif extension rather than .bin.",
    )
    parser.add_argument(
        "-resolution",
        "-r",
        type=float,
        default=None,
        help="Resolution (km): 6.25, 12.5, or 25. If omitted, it is interpreted from the file name. If cannot be interpreted, defaults to 25 km. Check your NSIDC data source documentation.",
    )
    parser.add_argument(
        "-hemisphere",
        type=str,
        default=None,
        help="Hemisphere: N or S. If omitted, it is interpreted from the file name. If cannot be interpreted, defaults to 'N'.",
    )
    parser.add_argument(
        "-nodata",
        "-nd",
        type=int,
        default=0,
        help="Nodata value. (Default: 0). IMPORTANT: If you want a nodata value other than 0, you should specify it. If you don't want to use a nodata value, then specify a nonsense value that is not contained in the data (such as -99999).",
    )
    parser.add_argument(
        "-header_size",
        "-hs",
        type=int,
        default=0,
        help="Size of .bin file header (in bytes.) (Default: 0)",
    )
    parser.add_argument(
        "-element_size",
        "-es",
        type=int,
        default=2,
        help="Size of each numerical .bin data element, in bytes. (Default: 2)",
    )
    parser.add_argument(
        "-output_type",
        "-ot",
        default="float",
        help="Output data type: 'int' or 'float'. Default 'float'.",
    )
    parser.add_argument(
        "-multiplier",
        "-m",
        type=str,
        default="auto",
        help="Use a multiplier. With 'auto', defaults to 1 for integers (no mod) and 0.1 for floating-point. If you want to use a different multiplier, put the number here.",
    )
    parser.add_argument(
        "--signed",
        "-s",
...
if __name__ == "__main__":
    ...
    if args.output_type.lower() in ("float", "f"):
        ...
        raise ValueError(
            "Unknown output_type (can be: 'int','i','d','float', or 'f'):",
